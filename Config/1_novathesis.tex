%!TEX root = ../template.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Config/1_novathesis.tex
%% NOVA thesis configuration file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\typeout{NT FILE Config/1_novathesis.tex}

%%============================================================
%%  TEMPLATE CUSTOMIZATION / OPTIONS
%%============================================================

%%-------------------------------- ----------------------------
%% THE MOST IMPORTANT/POPULAR OPTIONS
%%------------------------------------------------------------
\ntsetup{docdegree=msc}       % The type of document
%                               %     [ phd, phdplan, phdprop, msc, mscplan, bsc, plain ]
%                               % DEFAULT: docdegree=msc

\ntsetup{school=nova/fct}    % The school id
%                               %     [ nova/fct, nova/fcsh, nova/ims, nova/ensp, 
%                               %       ulisboa/ist, ulisboa/fc, 
%                               %       uminho/ea, uminho/ec, uminho/ed, uminho/ee, 
%                               %       uminho/eeg, uminho/em, uminho/ep, uminho/ese, 
%                               %       uminho/ics, uminho/ie, uminho/ilch, uminho/i3b, 
%                               %       iscteiul/eta, ips/ests, ipl/isel, ulht/deisi
%                               %       other/esep, other/mscgt ]
%                               % DEFAULT: school=nova/fct

\ntsetup{langsused={en,pt}}  % List of languages are used in the document
%                               %     [ en, pt, fr, it, de, es ]
%                               % DEFAULT: langsused=en,pt

\ntsetup{lang=en}             % The main language for the document text
%                               %     [ en, pt, fr, it, de, es ]
%                               % DEFAULT: lang=en

\ntsetup{glsxindy=false}       % Use an external glossaries processor (this method
%                               % is faster if you have more than a couple of entries)
%                               % DEFAULT: glsxindy=false

\ntsetup{linkscolor=DarkSlateGray}% The color for the hyperlinks (URLs, 
%                               %     cross references, citations)
%                               %     for valid color names, look at "svgname" 
%                               %     in the "xcolor" package documentation
%                               % DEFAULT: whatever hyperref uses as default

\ntsetup{media=screen}         % The target media for the PDF
%                               %     [ screen, paper ]
%                               %     SCREEN ==> left and right margins are 
%                               %                equal, colored links
%                               %     PAPER  ==> left and right margins are 
%                               %                 different, black links
%                               % DEFAULT: media=screen

\ntsetup{printfrontmatter=true}% Print the front matter
%                               %     set to false to generate a PDF with only the
%                               %     cover and the book chapters
%                               % DEFAULT: printfrontmatter=true

\ntsetup{printcommittee=true}% Shall the evaluation committee be printed?
%                               %     the evaluation committee should only be
%                               %     printed in final versions.
%                               % DEFAULT: printcommittee=false

% \ntsetup{gnumberlist=false}   % Shall the glossary entries list the page numbers where
%                               %     those entries are used?  (as a reverse index!)
%                               % DEFAULT: gnumberlist=true

% \ntsetup{secondcover=true}    % Shall a second cover page be forced?
%                               %     if the contents for the second page are not 
%                               %     defined, the second cover will be a replica 
%                               %     of the first cover.
%                               % DEFAULT: secondcover=false

\ntsetup{spine=false}          % Shall a “book spine” be printed at the end of
%                               %     the document?
%                               % DEFAULT: spine=false

\ntsetup{urlstyle=same}       % Use the same (main) font in URLs
%                               % DEFAULT: whatever hyperref uses as default


%\ntsetup{copyrightmodifier=by-sa-nc} % Modifier for the Creative Commons license.   
%                               % Ignored otherwise
%                               % DEFAULT: CC by-sa-nc license


%%------------------------------------------------------------
%% THE LESS IMPORTANT/POPULAR OPTIONS
%%------------------------------------------------------------
% \ntsetup{docstatus=provisional}   % The status of the document
%                               %     [ final, provisional, draft, working ]
%                               % DEFAULT: docstatus=final

% \ntsetup{tocintoc=true}       % Shall table of contects be listed in itself?
%                               % DEFAULT: tocintoc=false

% \ntsetup{fontstyle=erewhon}   % The font set to be used in the document
%                               %     [ bookman, erewho, kpfont, libertin,
%                               %       schola, stix2,
%                               %       newsgot(*), calibr(*), kieranhealy(*) ] 
%                               %     (*) available only for xelatex/lualatex)
%                               % DEFAULT: fontstyle=kpfonts

%\ntsetup{chapstyle=elegant}   % The chapter style to be used
%                               %     [ bar, bianchi, bluebox, compact, default, 
%                               %       elegant, ell, hansen, ist, lyhne, madsen, 
%                               %       pedersen, section, southall, veelo, vz34,
%                               %       vz43 ]
%                               % DEFAULT: chapstyle=bar

% \ntsetup{coverlang=en}        % The language for the cover (and second cover) page 
%                               %     defaults to main document language
%                               % DEFAULT: coverlang=the same as “lang”

% \ntsetup{copyrightlang=en}    % The language for the copyright message
%                               %     defaults to main document language
%                               % DEFAULT: copyrightlang=the same as “lang”

% \ntsetup{statement=true}     % Does the document have after cover pages??
%                               %     after cover pages are printed immediately 
%                               %     after the cover pages
%                               % DEFAULT: statement=false

%\ntsetup{printcopyright=false} % Shall the copyright message be printed??
%                               % DEFAULT: printcopyright=true

% \ntsetup{cdcover=true}        % Shall a “cd cover” be printed at the end of
%                               %     the document?
%                               % DEFAULT: cdcover=false

% \ntsetup{debugcover=true}     % Cover development and debug: print rulers in the cover page
%                               % DEFAULT: debugcover=false
